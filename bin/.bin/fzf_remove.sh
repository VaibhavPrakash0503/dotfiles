...
tmux display-popup -E -h 70% -w 60% -x C -y C "
  rpm -qa --qf '%{NAME}\n' | grep -v -E '^(lib|font|glibc|kernel|systemd|dbus|NetworkManager|grub|dracut)' | grep -v -E '(devel|debug|doc|lang|headers)\$' | sort | fzf --print-query \
      --bind='change:reload(if [ {q} ]; then rpm -qa --qf \"%{NAME}\\n\" | grep -i {q} | grep -v -E \"^(lib|font|glibc|kernel|systemd|dbus|NetworkManager|grub|dracut)\" | grep -v -E \"(devel|debug|doc|lang|headers)\\$\" | sort; else rpm -qa --qf \"%{NAME}\\n\" | grep -v -E \"^(lib|font|glibc|kernel|systemd|dbus|NetworkManager|grub|dracut)\" | grep -v -E \"(devel|debug|doc|lang|headers)\\$\" | sort ; fi)' \
